...
# --------------------
sample_dir="/Bulk/Imputation/Imputation from genotype (TOPmed)"
...
for bgen_file_dx in "${bgen_files[@]}"; do
    ...
    filename_prefix="${pgs_id}_${genome_build}_ukb21007_subset_chr${chr}${part}"
    ...
    if [ "$existing_count" -eq 3 ]; then
        echo "✅😇 All output files for chr${chr}${part} for ${genome_build} ${hg38_genotype_imputation_type} imputation already exist in ${output_folder}. Skipping."
        echo "-------------------------------------------"
        continue
    else
        echo "🟡 Missing outputs for chr${chr}${part} (${existing_count}/3 found for ${genome_build} ${hg38_genotype_imputation_type} imputation). Proceeding with conversion."
    fi

    # Corresponding UKB sample file path
    sample_file="ukb21007_c${chr}_b0_v1.sample"
    ...
    echo "😀 Submitting DNAnexus job for chr${chr}${part} for ${genome_build} ${hg38_genotype_imputation_type} imputation"
