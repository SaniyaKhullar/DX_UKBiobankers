...
pgs_id="PGS002724_PGS003725_PGS005097"
genome_build="GRCh37"
...
for snp_file_dx in "${snp_files[@]}"; do
    ...
    if [[ $bgen_exists -eq 1 ]]; then
        echo "✅😇 Output files for chr${chr}${part} already exist in ${output_folder}. Skipping."
        echo "-------------------------------------------"
        continue
    else
        echo "🟡 Missing outputs for chr${chr}${part}. Proceeding with extraction."
    ...
    # # --------------------
    # existing_count=$(dx ls -l "$output_folder" | awk '{print $NF}' | grep -E "^(${output_bgen})$" | wc -l)
    ...
    #     continue
    # else
    #     echo "🟡 Missing outputs for chr${chr}${part} (${existing_count}/1 found). Proceeding with extraction."
    # fi
    # # --------------------
    # # Check if output files already exist
    # # --------------------
    # existing_count=$(dx ls "${output_folder}/" | grep -E "^${output_bgen}$|^${output_bgen_index}$" | wc -l)
    ...
    #     continue
    # else
    #     echo "🟡 Missing outputs for chr${chr}${part} (${existing_count}/2 found). Proceeding with extraction."
    # fi
...
# # --------------------
# input_folder="/Bulk/Imputation/UKB imputation from genotype/"
# output_folder="combined_saniya_demo/extracted_genotypes_bgen"
# pgs_id="PGS005097"
# genome_build="GRCh37"
# snp_folder="combined_saniya_demo/pgs_snps"
# log_file="variant_counts_${pgs_id}_${genome_build}.log"
...
# # --------------------
# snp_files=( $(dx ls combined_saniya_demo/pgs_snps --brief) )
# # snp_files=()
# # while IFS= read -r line; do
# #     snp_files+=("$line")
# # done < <(dx ls "$snp_folder" --brief)
...
# output_folder="/combined_saniya_demo/extracted_genotypes_bgen"
# pgs_id="PGS005097"
# chrom_file="/combined_saniya_demo/pgs_snps/chromNames_${pgs_id}.txt"
# genome_build="GRCh37"
# log_file="variant_counts_${pgs_id}_${genome_build}.log"
...
#     snp_file_dx="/combined_saniya_demo/pgs_snps/${snp_filename}" # Check if SNP file exists
